...
mp::PlainSSHSession::PlainSSHSession(const std::string& host,
    ...
    : raw_session{ssh_new(), ssh_free}, mut{}
{
    ...
    SSH::throw_on_error(raw_session, "ssh connection failed", ssh_connect);
...
mp::PlainSSHSession::~PlainSSHSession()
{
    top_catch_all(category, [this] {
        ...
        if (raw_session)
        {
            ...
            ssh_disconnect(raw_session.get());
...
void mp::PlainSSHSession::force_shutdown()
{
    ...
    if (auto socket = ssh_get_fd(raw_session.get()); socket != -1)
...
void mp::PlainSSHSession::set_option(ssh_options_e type, const void* data)
{
    ...
    const auto ret = ssh_options_set(raw_session.get(), type, data);
    if (ret != SSH_OK)
    {
        throw mp::SSHException(fmt::format("libssh failed to set {} option to '{}': '{}'",
                                           name_for(type),
                                           as_string(type, data),
                                           ssh_get_error(raw_session.get())));
    }
}
...
{
    std::unique_lock lock{mut};
    return raw_session && static_cast<bool>(ssh_is_connected(raw_session.get()));
}
