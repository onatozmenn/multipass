...
struct SshfsMount : public mp::test::SftpServerTest
{
    ...
    // The 'invoked' parameter binds the execution and read mocks. We need a better mechanism to
    // make them cooperate better, i.e., make the reader read only when a command was issued.
    auto make_exec_to_check_commands(const CommandVector& commands,
                                     std::string::size_type& remaining,
                                     CommandVector::const_iterator& next_expected_cmd,
                                     std::string& output,
                                     bool& invoked,
                                     std::optional<std::string>& fail_cmd,
                                     std::optional<bool>& fail_invoked)
    {
        ...
        auto request_exec = [this,
            ...
            {
                // Check if the first element of the given commands list is what we are expecting.
            ...
            // If the command list was entirely checked or if the executed command is not on the
            // list, check the default commands to see if there is an answer to the current command.
    ...
    auto make_channel_read_return(const std::string& output,
                                  std::string::size_type& remaining,
                                  bool& prereq_invoked)
    {
        auto channel_read =
    ...
    void test_command_execution(const CommandVector& commands,
                                ...
                                std::optional<bool> fail_invoked = std::nullopt)
    {
        ...
        EXPECT_TRUE(next_expected_cmd == commands.end())
...
// Mocks correct execution of a given vector of commands/answers. The first string specifies the
// parameter with which make_sshfsmount mock is called.
struct SshfsMountExecute : public SshfsMount,
...
struct SshfsMountExecuteThrowInvArg : public SshfsMount,
...
TEST_P(SshfsMountFail, testFailedInvocation)
{
    ...
    auto request_exec = make_exec_to_check_commands(empty,
                                                    remaining,
                                                    it,
                                                    output,
                                                    invoked_cmd,
...
INSTANTIATE_TEST_SUITE_P(SshfsMountThrowWhenError,
                         SshfsMountFail,
...
INSTANTIATE_TEST_SUITE_P(SshfsMountSuccess,
                         SshfsMountExecute,
                         testing::Values(std::make_pair("target", old_fuse_cmds),
                                         std::make_pair("target", new_fuse_cmds),
                                         std::make_pair("target", unk_fuse_cmds),
                                         std::make_pair("target", exec_cmds),
                                         std::make_pair("space odyssey", space_cmds),
...
TEST_F(SshfsMount, blankFuseVersionLogsError)
{
    ...
    EXPECT_CALL(*logger_scope.mock_logger,
                log(Eq(mpl::Level::warning),
                    StrEq("sshfs mount"),
                    StrEq("Unable to parse the FUSE library version")));
    EXPECT_CALL(*logger_scope.mock_logger,
